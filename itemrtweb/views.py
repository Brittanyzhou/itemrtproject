...
from itemrtproject import assessment_engine, formatter_engine,programming,tester
    ...
    if request.method == 'POST':
        ...
        for question in test.questions.all():
            try:
                ...
                test_response = TestResponse.objects.filter(test=test).filter(user=request.user).filter(question=question)
                pass
...
def prototype(request, question_id=None):
    ...
    assert isinstance(question_id, object,)
    if question_id:
    ...
    return render(request, 'itemrtweb/manage.question.form.html', {'form': form, 'topics': topics, 'selected_question': selected_question})
@user_passes_test(lambda u: u.is_staff)
def prototype1(request, question_id=None):
    # Init
    selected_question = None
    # Obtain the list of topics
    topics = Topic.objects.all()
    ...
    assert isinstance(question_id,  object,)
    if question_id:
        selected_question = LongQuestion.objects.all().get(id=int(question_id))
    if request.method == 'POST':
        form = forms.InsertEditQuestionForm1(request.POST)  # Bind to user submitted form
        if form.is_valid():
                # Insert new question
                question = LongQuestion(content=form.cleaned_data['content'], difficulty=form.cleaned_data['difficulty'],topic=form.cleaned_data['topic'],)
                question.save()
                global PROG_ID
                PROG_ID = question.id
                # Insert answer for question
                #programmcase = ProgramCase(question=question, programming_inputs=form.cleaned_data['programming_inputs'], expected_outputs=form.cleaned_data['expected_outputs'],)
                #programmcase.save()
                # Insert solution if exists
                if form.cleaned_data['solution']:
                    solution = Solution1(question=question, content=form.cleaned_data['solution'])
                    solution.save()
                for tag_name in form.cleaned_data['tags']:
                    # Check tag exists in db, otherwise add
                    # Currently no need since automatically verified
                    ...
                    qn_tag = QuestionTag(question=question, tag=tag)
                    qn_tag.save()
                return HttpResponseRedirect("/programming_answer/")
            #return redirect('/prototype4/list/topic/'+ str(question.topic.id) +'/?msg=Question '+ str(question.id) +' added successfully#question-'+ str(question.id))
        # Reply regardless valid
        return render(request, 'itemrtweb/manage.programmingquestion.form.html', {'form': form, 'topics': topics, 'selected_question': selected_question,})
        #return render_to_response('itemrtweb/manage.programminganswer1.form.html',{'form': form, 'topics': topics, 'selected_question': sioelected_question})
    else:

        # Check if question exists or give blank form
        if selected_question:
            # Load existing question into a form
            form = forms.InsertEditQuestionForm1(initial={'content':selected_question.content, 'difficulty':selected_question.difficulty, 'topic':selected_question.topic,'tags': selected_question.tags.all().values_list('name', flat=True)})

            solution_exists = Solution1.objects.all().filter(question=selected_question).count()

            if solution_exists > 0:
                form.fields["solution"].initial = selected_question.solution1.content

        else:
            # Display new form for user to fill in
            form = forms.InsertEditQuestionForm1()

    return render(request, 'itemrtweb/manage.programmingquestion.form.html', {'form': form, 'topics': topics, 'selected_question': selected_question,})

@user_passes_test(lambda u: u.is_staff)
def programming_answer(request,programcase_id=None,):
    # Init
    programcase = None
    global PROG_ID
    question_id = PROG_ID
    selected_question = LongQuestion.objects.all().get(id=question_id)
    assert isinstance(question_id,  object,)
    if question_id:
        selected_question = LongQuestion.objects.all().get(id=int(question_id))
    if programcase_id:
        programcase=ProgramCase.objects.all().get(id=programcase_id)
    # Objectify the selected question
    if request.method == 'POST':
        form = forms. ProgrammingAnswerForm(request.POST)  # Bind to user submitted form
        if form.is_valid():
                # Insert new programcase
                programcase = ProgramCase(question=selected_question, programming_inputs=form.cleaned_data['programming_inputs'], expected_outputs=form.cleaned_data['expected_outputs'],)
                programcase.save()
                global CASE_ID
                CASE_ID = programcase.id
                # Insert programming_inputs if exists
                #return HttpResponseRedirect("/prototype1a/")
                return redirect('/prototype4/list/topic/'+ str(selected_question.topic.id) +'/?msg=Question '+ str(selected_question.id) +' edited successfully#question-'+ str(selected_question.id))
               # return HttpResponseRedirect("/prototype1a/")
        # Reply regardless valid
        return render(request, 'itemrtweb/manage.programminganswer1.form.html', {'form': form, 'selected_quesiton': selected_question})

    else:
        # Check if question exists or give blank form
        if programcase:
            # Load existing question into a form
            form = forms.ProgrammingAnswerForm(initial={'programing_inputs':programcase.programing_inputs, 'expected_outputs':programcase.expected_outputs})
        else:
            # Display new form for user to fill in
            form = forms.ProgrammingAnswerForm()

    return render(request, 'itemrtweb/manage.programminganswer1.form.html', {'form': form,  'programcase': programcase,})
@user_passes_test(lambda u: u.is_staff)
def prototype1a(request, question_id=None,):
    # Init
    selected_question= None
    exist_programming_inputs=None
    exist_expected_outputs=None
    exist_marks=None
    #programcase=None
    filtered_programcase=None
    # Obtain the list of topics
    topics = Topic.objects.all()
    # Objectify the selected question
    assert isinstance(question_id,  object,)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            if selected_question:
                ...
                if form.cleaned_data['solution']:
                    ...
                    if solution_exists > 0:
                        # Update solution
                        solution = selected_question.solution1
                        solution.content = form.cleaned_data['solution']
                        solution.save()
                    else:
                        # New solution for this question
                        solution = Solution1(question=selected_question, content=form.cleaned_data['solution'])
                        solution.save()
            ...
            #return redirect('/prototype4/list/topic/'+ str(question.topic.id) +'/?msg=Question '+ str(question.id) +' added successfully#question-'+ str(question.id))
        # Reply regardless valid
        return render(request, 'itemrtweb/manage.programmingeditquestion.form.html', {'form': form, 'topics': topics, 'selected_question': selected_question,'programmcase': filtered_programcase,'programming_inputs':exist_programming_inputs,'expected_outputs':exist_expected_outputs,'marks':exist_marks})
        #return render_to_response('itemrtweb/manage.programminganswer1.form.html',{'form': form, 'topics': topics, 'selected_question': sioelected_question,})
    else:
        ...
        if selected_question:
            ...
            filtered_programcase = ProgramCase.objects.all().filter(question=selected_question)
            if filtered_programcase>0:
             programcase = selected_question.programmecase.all()[0]
             exist_programming_inputs =programcase.programming_inputs
            # exist_programming_inputs.save()
             exist_expected_outputs=programcase.expected_outputs
            # exist_expected_outputs.save()
             exist_marks=programcase.marks
             #exist_marks.save()
    ...
    return render(request, 'itemrtweb/manage.programmingeditquestion.form.html', {'form': form, 'topics': topics, 'selected_question': selected_question,'programmcase': filtered_programcase,'programming_inputs':exist_programming_inputs,'expected_outputs':exist_expected_outputs,'marks':exist_marks})
    #return render(request, 'itemrtweb/manage.programmingeditquestion.form.html', {'form': form, 'topics': topics, 'selected_question': selected_question,'programmcase': programmcase,})
@user_passes_test(lambda u: u.is_staff)
def programming_answer_edit(request,programcase_id=None):
    # Init
    programcase = None
    global SQ_ID
    question_id = SQ_ID
    ...
    if request.method == 'POST':
        ...
        if programcase:
            # Load existing question into a form
            form = forms.ProgrammingAnswerForm(initial={'programming_inputs':programcase.programming_inputs, 'expected_outputs':programcase.expected_outputs})
        else:
            # Display new form for user to fill in
            form = forms.ProgrammingAnswerForm()
...
@user_passes_test(lambda u: u.is_staff)
def programming_answer_delete(request, programcase_id=None):
    # Init
    programcase = None

    # Objectify the selected question
    if programcase_id:
        programcase = ProgramCase.objects.all().get(id=int(programcase_id))

    # Check if question exists otherwise redirect to question list
    if programcase:
        # Hide question and save. Then give message to user
        programcase.is_save = False
        programcase.save()
        return redirect('/"/prototype1/"/?msg=Answer has been deleted')
    else:
        # Redirect user back to question lists
        return redirect('/prototype1/')
@user_passes_test(lambda u: u.is_staff)
...
@user_passes_test(lambda u: u.is_staff)
def prototype6(request, question_id=None):
    # Init
    selected_question = None
    ...
    if selected_question:
        ...
        return redirect('/prototype4/?msg=Question has been deleted')
    else:
        # Redirect user back to question lists
        return redirect('/prototype1/')
...
@user_passes_test(lambda u: u.is_staff)
def prototype4(request, topic_id=None):
    # Record usage for stats purpose
    page = "question_management"
    # Never accessed this page before, or last access was more than 10 mins ago
    if 'user_usage_'+page not in request.session or datetime.now() > datetime.strptime(request.session['user_usage_'+page], "%a %b %d %H:%M:%S %Y") + timedelta(minutes=10):
    ...
    if topic_id:
        # Retrieve questions for this topic
        filtered_questions = LongQuestion.objects.all().filter(topic=topic_id)
    ...
    return render(request, 'itemrtweb/manage.programmingquestion.list .html', {'topics': topics, 'selected_topic': selected_topic, 'questions': filtered_questions, 'all_tags': all_tags})
@user_passes_test(lambda u: u.is_staff)
...
@user_passes_test(lambda u: u.is_staff)
def prototype4a(request):
    # Init
    filtered_questions = LongQuestion.objects.all()
    ...
    return render(request, 'itemrtweb/manage.programmingquestion.list .html', {'topics': topics, 'questions': filtered_questions, 'tags': tags, 'all_tags': all_tags})
...
def preview(request, question_id=None):
    ...
    if selected_question:
        ...
        raise Http404
@user_passes_test(lambda u: u.is_staff)
def preview1(request, question_id=None):
    # Init
    selected_question = None
    ...
    if question_id:
        selected_question = LongQuestion.objects.all().get(id=int(question_id))
        # Get correct answer
       # selected_question.solution = selected_question.solution.all()[0]

    # Check if question exists otherwise redirect to question list
    if selected_question:
        return render(request, 'itemrtweb/manage.programmingquestion.preview.html', {'question': selected_question})
    else:
        # 404 if question was not found
        raise Http404
#programming questions contains online programming
@login_required
def programming(request, topic_id=None):
    # Record usage for stats purpose
    page = "question_management"
    # Never accessed this page before, or last access was more than 10 mins ago
    if 'user_usage_'+page not in request.session or datetime.now() > datetime.strptime(request.session['user_usage_'+page], "%a %b %d %H:%M:%S %Y") + timedelta(minutes=10):
   ...
   # all_tags = Tag.objects.all()
    ...
    return render(request, 'itemrtweb/manage.programmingtest.list.html', {'topics': topics, 'selected_topic': selected_topic, 'questions': filtered_questions})
@login_required
def long_question_view(request,question_id=None,):
    ...
    if selected_question:
        formatter = formatter_engine.WebQuestionFormatter()
        selected_question = formatter.format(selected_question)
        # Temp variable to allow ajax through https
        host = request.get_host()
        is_secure = not "localhost" in host
        return render(request, 'itemrtweb/programmingquestion.view.html', {'question': selected_question})
    else:
        # 404 if question was not found
        raise Http404
@login_required
def programming_submit(request,question_id,):
    selected_question=LongQuestion.objects.get(id=question_id)
    if 'ans' in request.POST and request.POST['ans']:
        user=request.user
        if selected_question:
            try:
                # Previously saved response available? OK do nothing for now.
               submit_response = ProgrammingResponse.objects.filter(user=request.user).get(question=selected_question)
               submit_response.response = request.POST['ans']
               submit_response.save()
               pass
            # Otherwise new response, create object
            except ObjectDoesNotExist:
              submit_response = ProgrammingResponse(user=request.user, question=selected_question, response=request.POST['ans'],)
              submit_response.save()
            #formatter = formatter_engine.WebQuestionFormatter()
            #selected_question=formatter.format(selected_question)
            selected_question.content = re.sub(r'\\begin{code}\r\n', '<pre class="prettyprint linenums">', selected_question.content)
            selected_question.content = re.sub(r'\\end{code}\r\n', '</pre>', selected_question.content)
            index_start_pre = selected_question.content.find('<pre class="prettyprint linenums">')
            index_end_pre = selected_question.content.find('</pre>')
            safe_string = selected_question.content[index_start_pre+34:index_end_pre]
            string=safe_string.replace(" /*add your code here*/",submit_response.response)
            #code=safe_string.replace(" /*add your code here*/",submit_response.response)
            #string=unescape(code)
            #class_name=get_java_class_name(code=selected_question.content)
            #os.getcwd()
            #fo = open(class_name+".java", "wb")
            #fo.write(string)
            # close file
           # fo.close()
            #formatter = formatter_engine.WebQuestionFormatter()
            #selected_question=formatter.format(selected_question)
            programtester=tester.Programtest(string,user,selected_question)
            try:
             programtester.compile()
            except Exception as e:
                # when there is compilation error, stay in the same page and show user the error via 'message'
                    message=e.message
                    return render_to_response("itemrtweb/programmingquestion.view.html",{'question':selected_question,"message":message},context_instance=RequestContext(request))
            # run test
            result_brief,result_detail=programtester.run_test
            detail_info="Brief result:<br/>"+result_brief + "<br>" + "Detailed result:<br>" + result_detail
            #save result
            #programtester.save_result()
            #formater solution
            formatter = formatter_engine.WebQuestionFormatter()
            selected_question.solution1=formatter.format(selected_question.solution1)
            return render(request, 'itemrtweb/programming.submit.html', {'question': selected_question, 'detail': detail_info, 'solution': selected_question.solution1})
    return render(request, 'itemrtweb/programming.submit.html', {'question': selected_question, 'solution': selected_question.solution1})
@login_required
