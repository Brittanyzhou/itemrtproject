...
class Topic(models.Model):
    ...
    def __unicode__(self):
        return self.name
# New in version 20141215
# Added description field in version  20141215
...
#New in version 20150202
class LongQuestion(models.Model):
    "LongQuestion model for Programming, fill-in-the-blanks questions with 1 part"
    content     = models.TextField(max_length=12000)
    difficulty  = models.IntegerField()
    topic       = models.ForeignKey(Topic)
    time        = models.IntegerField(default=0) # In seconds
    type        = models.ForeignKey(Type, default=1)
    meta        = models.ManyToManyField(Meta, through='LongQuestionMeta')
    is_active   = models.BooleanField(default=True) # Deleted will be false
    tags        = models.ManyToManyField('Tag', through='LongQuestionTag')
    marks       = models.DecimalField(max_digits=3, decimal_places=1, default=1.0)
    def _get_question(self):
        "Get text of the question (without input information)"
        index = self.content.find('sample')
        return self.content[0:index]
    text        = property(_get_question)
    #choices     = property(numSpaces)
    objects_all = models.Manager()
    objects     = ActiveQuestionManager()
    class Meta:
        ordering = ['id']
    ...
    def __unicode__(self):
        return "LongQuestion " + str(self.id)
...
class LongQuestionTag(models.Model):
    question    = models.ForeignKey(LongQuestion)
    tag         = models.ForeignKey(Tag)
# New in version 20141215
...
#New in version 20150205
class Solution1(models.Model):
    "Solution model for question solutions if any"
    question    = models.OneToOneField(LongQuestion, primary_key=True, related_name='solution1')
    content     = models.TextField(max_length=2000)
    rating      = models.IntegerField(blank=True, null=True)
#New in version
# New in version 20141215
class Answer(models.Model):
    ...
    correctness = models.DecimalField(max_digits=3, decimal_places=2, default=1)
     # Ans can be still correct but not 100% correct
class ActiveProgramcaseManager(models.Manager):
    def get_query_set(self):
        return super(ActiveProgramcaseManager, self).get_query_set().filter(is_save=True)
class ProgramCase(models.Model):
    "Test case for long question answer input code"
    question = models.ForeignKey(LongQuestion, related_name='programmecase')
    programming_inputs = models.TextField(max_length=1000)
    expected_outputs= models.TextField(max_length=1000)
    marks       = models.DecimalField(max_digits=3, decimal_places=1, default=1.0)
    is_save   = models.BooleanField(default=True) # Deleted will be false
    result_detail =models.TextField(max_length=1000)
class ProgramResult(models.Model):
    question = models.ForeignKey(LongQuestion,related_name='programresult')
    actual_output_list = models.TextField(max_length=1000)
    actual_error_list = models.TextField(max_length=1000)
...
class ProgrammingResponse(models.Model):
    "Response model to store user programming response"
    user = models.ForeignKey(User)
    question = models.ForeignKey(LongQuestion)
    response = models.TextField(max_length=2000)
    date = models.DateTimeField(auto_now=True)
    #duration = models.IntegerField(blank=True, null=True) # In
    #correctness = models.DecimalField(max_digits=3, decimal_places=2, null=True) # Percent correct in dec (0-1)
   # criterion   = models.DecimalField(max_digits=3, decimal_places=1) # Max marks for random practice/test, diff for CAT
   # ability     = models.DecimalField(max_digits=5, decimal_places=2, null=True) # Current ability score for practices
# New in version 20141215
...
#New in version 20150202
class LongQuestionMeta(models.Model):
    "LongQuestionMeta model is an intermediary model between Question and Meta models"
    question    = models.ForeignKey(LongQuestion)
    meta        = models.ForeignKey(Meta)
    content     = models.CharField(max_length=50)
# New in version 20141215
